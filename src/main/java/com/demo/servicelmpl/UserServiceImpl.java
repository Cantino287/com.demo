...
//    }
        ...
        try {
            ...
            if (userObj != null) {
                ...
                if (userObj.getPassword().equals(oldPassword)) {
                    userObj.setPassword(newPassword);  // ⚠️ Hashing recommended!
                    userDao.save(userObj);
                    return ResUtils.getResponseEntity("Password Updated Successfully", HttpStatus.OK);
                } else {
                    return ResUtils.getResponseEntity("Incorrect Old Password", HttpStatus.BAD_REQUEST);
                ...
                return ResUtils.getResponseEntity("User not found", HttpStatus.NOT_FOUND);
            }
        } catch (Exception ex) {
            ex.printStackTrace();
            return ResUtils.getResponseEntity(ResConstants.SOMETHING_WENT_WRONG, HttpStatus.INTERNAL_SERVER_ERROR);
    ...
    public ResponseEntity<String> resetPassword(Map<String, String> requestMap) {
        ...
        if (!otpStorage.containsKey(email)) {
            return new ResponseEntity<>("No OTP found for this email.", HttpStatus.BAD_REQUEST);
        }
        ...
        user.setPassword(newPassword);  // ⚠️ Hash this password!
        userDao.save(user);

        otpStorage.remove(email); // Clear used OTP
